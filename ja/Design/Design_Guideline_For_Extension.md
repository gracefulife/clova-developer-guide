...
* 1つのユーザー・アクション単位で、1つの具体的な目標を作成することをお勧めします。
...
| #3         | メニューの照会および選択      | ユーザーはピザのメニューを照会して選択できる。                                   |
| #4         | サービス提供者の照会および選択     | ユーザーはピザのサービス提供者を選択できる。                                 |
...
ピザの宅配サービス(Extension)の対話モデルを定義するということは、メニュー照会インテント、注文インテント、配達状況確認インテントのようにインテントのリストを宣言して、それぞれのインテントが必要とする情報(スロット)とサンプル発話を列挙するということです。従って、**対話モデルを定義する際、一番最初に、Extensionが処理するリクエストのカテゴリーを定義し、列挙する**必要があります。それはExtensionを開発する際に、ビジネスロジックでプログラムの分岐を分ける基準にもなります。
...
| CLOVA.MONEY          | 数字+貨幣表現を提供します。(例:"四千円"、"1ドル") |
