# CarRoute Template
자동차 길찾기 결과 데이터를 제공하는 템플릿입니다. 화면에 자동차 길찾기 결과를 표시할 때 사용됩니다. 자동차 길찾기 템플릿을 표시하려면 {{book.OrientedService}} 지도 API에 대한 이해가 필요하며, 템플릿으로 전달된 데이터와 {{book.OrientedService}} 지도 API를 이용하여 길찾기 결과를 지도에 표시해야 합니다. 네이버 지도 API에 대한 자세한 설명은 [{{book.OrientedService}} 지도 API 문서](https://navermaps.github.io/maps.js/docs/)를 참고합니다. 길찾기 결과를 표시한 예는 [Screen UI example](#UIExample)을 참조합니다.
...
| summary.roadSummary[].roadCongestion | [StringObject](/CIC/References/ContentTemplates/Shared_Objects.md#StringObject) | 도로 상황 정보가 담긴 객체. *summary.roadSummary.roadCongestion.value*는 다음과 같은 값을 가질 수 있습니다. <ul><li>"0" : 미수신</li><li>"1" : 원활</li><li>"2" : 서행</li><li>"3" : 지체</li><li>"4" : 정체</li></ul> | 필수 |
...
| summary.start.lat                    | [StringObject](/CIC/References/ContentTemplates/Shared_Objects.md#StringObject) | 도착지의 위도 정보가 담긴 객체 | 필수 |
| summary.start.lon                    | [StringObject](/CIC/References/ContentTemplates/Shared_Objects.md#StringObject) | 도착지의 경도 정보가 담긴 객체 | 필수 |
| summary.start.name                   | [StringObject](/CIC/References/ContentTemplates/Shared_Objects.md#StringObject) | 도착지의 이름 정보가 담긴 객체 | 필수 |
| summary.time                         | [StringObject](/CIC/References/ContentTemplates/Shared_Objects.md#StringObject) | 이동 시간 정보가 담긴 객체. 단위는 분입니다. | 필수 |
