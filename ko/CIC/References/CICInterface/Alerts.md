...
3. 클라이언트가 알람 등록을 시도한 후 그 결과를 CIC에게 전달합니다.([`Alerts.SetAlertSucceeded`](#SetAlertSucceeded) 이벤트 메시지, [`Alerts.SetAlertFailed`](#SetAlertFailed) 이벤트 메시지 사용)
...
3. 클라이언트가 알람을 수정하거나 삭제한 그 결과를 CIC에게 전달합니다. (관련 이벤트 메시지 사용)
...
클라이언트에게 `payload` 필드에 있는 사용자의 알람 데이터를 동기화하도록 지시합니다. 클라이언트는 CIC로부터 전달된 데이터에 맞게 클라이언트에 설정된 알람 값을 변경해야 합니다.
