...
사용자가 클라이언트 기기의 단축 버튼 중 하나를 누르거나 터치한 경우 클라이언트는 이 이벤트 메시지를 CIC에게 전송해야 합니다. Clova는 사용자가 클라이언트에 설정한 임의의 동작을 수행해줍니다. 예를 들어, 클라이언트가 팟캐스트와 같은 extension을 사용하고 있었다면 Clova는 사용자가 지정한 팟캐스트의 최신 에피소드를 재생할 수 있도록 처리해줍니다.
