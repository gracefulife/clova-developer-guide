...
| `initiator.payload.wakeWord.indices.endIndexInSamples`    | number   | 오디오 스트림에서 호출어가 끝나는 시점의 index 정보. 음성 입력이 16 kHz sample rate를 가지므로 index의 1 단위는 16,000 분의 1초를 의미합니다. 호출어에 해당하는 구간이 전체 오디오 스트림의 재생 시간 중 0에서 1초 사이에 위치한다면 호출어가 끝나는 시점의 index 값으로 16000을 입력해야 합니다.  | 필수  |
