...
|---------------|---------|-----------------------------|---------|
| `expectSpeechId`        | string  | 사용자의 음성 입력을 추가로 받을 때 CIC가 이를 식별하기 위한 ID. 추후 이 값은 사용자의 추가 음성 입력을 [`SpeechRecognizer.Recognize`](#Recognize) 이벤트 메시지를 CIC로 보낼 때 `speechId` 필드에 입력되어야 합니다.    | 필수 |
