...
| [Get](#Get)         | Event  | CIC에 사용자가 생선한 모든 리마인더 목록을 요청합니다.  |
...
4. 클라이언트는 사용자로부터 최종 확인을 받습니다.
   * 사용자가 리마인더 생성을 수락한 경우: Reminder.Created 이벤트 메시지를 CIC에 전송합니다.
   * 사용자가 리마인더 생성을 거부한 경우: 따로 이벤트 메시지를 보내지 않아도 됩니다.
...
4. 클라이언트는 사용자로부터 최종 확인을 받습니다.
   * 사용자가 리마인더 삭제를 수락한 경우: Reminder.Deleted 이벤트 메시지를 CIC에 전송합니다.
   * 사용자가 리마인더 삭제를 거부한 경우: 따로 이벤트 메시지를 보내지 않아도 됩니다.
...
4. 클라이언트는 사용자로부터 최종 확인을 받습니다.
   * 사용자가 리마인더 조회를 수락한 경우: Reminder.Get 이벤트 메시지를 CIC에 전송합니다.
   * 사용자가 리마인더 조회를 거부한 경우: 따로 이벤트 메시지를 보내지 않아도 됩니다.
...
* 각 리마인더의 필드 값은 [Reminder.Get](#Get) 이벤트 메시지 응답 전달되는 [Clova.RenderReminderList](/CIC/References/APIs/Clova.md#RenderReminderList) 지시 메시지를 통해 확인할 수 있습니다.
