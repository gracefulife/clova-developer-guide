...
4. 클라이언트는 사용자로부터 최종 확인을 받습니다.
   * 사용자가 메모 생성을 수락한 경우: Memo.Created 이벤트 메시지를 CIC에 전송합니다.
   * 사용자가 메모 생성을 거부한 경우: 따로 이벤트 메시지를 보내지 않아도 됩니다.
...
4. 클라이언트는 사용자로부터 최종 확인을 받습니다.
   * 사용자가 메모 삭제를 수락한 경우: Memo.Deleted 이벤트 메시지를 CIC에 전송합니다.
   * 사용자가 메모 삭제를 거부한 경우: 따로 이벤트 메시지를 보내지 않아도 됩니다.
...
4. 클라이언트는 사용자로부터 최종 확인을 받습니다.
   * 사용자가 메모 조회를 수락한 경우: Memo.Get 이벤트 메시지를 CIC에 전송합니다.
   * 사용자가 메모 조회를 거부한 경우: 따로 이벤트 메시지를 보내지 않아도 됩니다.
