...
| `intent`        | object  | 사용자의 요청을 분석한 정보가 저장된 객체 [intent](/DevConsole/Guides/CEK/Define_Interaction_Model.md#Intent)                          | 필수 |
| `intent.name`   | string  | Intent 이름. Interaction 모델에 정의한 [intent](/DevConsole/Guides/CEK/Define_Interaction_Model.md#Intent)를 이 필드로 식별할 수 있다.  | 필수 |
| `intent.slots`  | object  | Extension이 intent를 처리할 때 요구되는 정보(slot)가 저장된 객체. 이 필드는 `intent.name` 필드에 입력된 [intent](/DevConsole/Guides/CEK/Define_Interaction_Model.md#Intent)에 따라 구성이 달라질 수 있다. | 필수 |
...
| 필드 이름       | 자료형    | 필드 설명                     | 필수 여부 |
|---------------|---------|-----------------------------|---------|
| `type`          | string  | 요청 메시지의 타입. `"SessionEndedRequest"` 값으로 고정됩니다. | 필수 |
